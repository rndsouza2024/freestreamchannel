...
const MovieCard: React.FC<MovieCardProps> = ({ item }) => {
  ...
  return (
    <Link to={`/watch/${type}/${item.id}`} className="group relative block h-full">
      <div className="relative aspect-[2/3] overflow-hidden rounded-xl bg-miraj-gray border border-white/5 shadow-lg transition-transform duration-300 md:group-hover:scale-105 group-hover:shadow-miraj-red/20 group-hover:border-miraj-gold/30">
        ...
        {/* Hover Overlay - Visible on hover or active (mobile tap) */}
      ...
      <div className="mt-2 space-y-1">
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          <span>{year}</span>
          <span className="border border-white/10 px-1.5 py-0.5 rounded uppercase text-[10px] tracking-wider">{type.replace('_', ' ')}</span>
        </div>
      </div>
    </Link>
  );
