...
import React, { useEffect, useState } from 'react';
import MediaCard from '../components/MediaCard';
import { ChevronRight, RefreshCw, TrendingUp, Film, Tv, Zap, Wifi, Loader2, Globe, Shield, Clock, Search, Play } from 'lucide-react';
import { fetchTrending, fetchMovies, fetchTVShows, fetchSports, fetchTVLive } from '../services/tmdb';
import { MediaItem } from '../types';
  ...
  return (
    <div className="min-h-screen bg-dark-bg pt-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
          <div className="text-center">
            ...
            <div className="mb-8 sm:mb-10 max-w-6xl mx-auto">
              <div className="bg-gradient-to-b from-dark-surface/50 to-dark-surface/30 border border-brand-500/20 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-xl shadow-brand-900/30">
                
                {/* Introduction Section */}
                <div className="mb-6 sm:mb-8">
                  {/* <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-3 mb-4 sm:mb-6">
                    <div className="flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-r from-brand-500 to-blue-500 rounded-xl sm:rounded-2xl">
                    ...
                    <div className="text-center sm:text-left">
                      ...
                      <div className="w-16 sm:w-20 h-1 bg-gradient-to-r from-brand-500 to-blue-500 mx-auto sm:mx-0 rounded-full"></div>
                    </div>
                  </div> */}
                </div>
                ...
                <div className="space-y-4 sm:space-y-6 text-left">
                  {/* Section 1 */}
                  <div className="bg-dark-bg/40 p-4 sm:p-5 rounded-xl border border-dark-border">
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="flex-shrink-0 mt-1">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-brand-500 to-brand-600 rounded-lg flex items-center justify-center">
                  ...
                  <div className="bg-gradient-to-r from-purple-900/30 to-blue-900/30 p-4 sm:p-5 rounded-xl border border-purple-500/30">
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="flex-shrink-0 mt-1">
                      ...
                      <div>
                        ...
                        <div className="space-y-3 sm:space-y-4">
                          <p className="text-gray-300 leading-relaxed text-sm sm:text-base md:text-lg">
                            From a legal perspective, Uwatchfree™ operates strictly as an indexing and aggregation service. All media links displayed on the site are sourced from external platforms and are provided "as is." Uwatchfree™ does not claim ownership of any content and does not control, host, or modify third-party streams. Copyright responsibility remains with the original content providers and hosting platforms.
                          </p>
                          <div className="bg-dark-bg/50 p-3 sm:p-4 rounded-lg border border-dark-border">
                            <p className="text-xs sm:text-sm text-gray-400 italic">
                              Users are encouraged to respect their local laws and the terms of service of any third-party websites they choose to access.
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                {/* CTA Section */}
                <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-dark-border text-center">
                  <div className="inline-flex flex-col sm:flex-row items-center gap-3 sm:gap-4 bg-gradient-to-r from-brand-500/20 to-blue-500/20 px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-full w-full sm:w-auto">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs sm:text-sm text-gray-400">
                      Choose from thousands of free titles
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-3 sm:gap-4 justify-center">
              <button 
                onClick={() => window.location.hash = '#/movies'}
                className="group flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white px-4 sm:px-6 py-3 rounded-lg sm:rounded-xl font-semibold transition-all hover:scale-105 text-sm sm:text-base"
              >
                <Film size={18} />
                <span>Browse Movies</span>
              </button>
              <button 
                onClick={() => window.location.hash = '#/tv'}
                className="group flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-white px-4 sm:px-6 py-3 rounded-lg sm:rounded-xl font-semibold transition-all hover:scale-105 text-sm sm:text-base"
              >
                <Tv size={18} />
                <span>TV Shows</span>
              </button>
              <button 
                onClick={() => window.location.hash = '#/sports'}
                className="group flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 sm:px-6 py-3 rounded-lg sm:rounded-xl font-semibold transition-all hover:scale-105 text-sm sm:text-base"
              >
                <Zap size={18} />
                <span>Live Sports</span>
              </button>
              <button 
                onClick={() => window.location.hash = '#/iptv'}
                className="group flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 sm:px-6 py-3 rounded-lg sm:rounded-xl font-semibold transition-all hover:scale-105 text-sm sm:text-base"
              >
                <Wifi size={19} />
                <span>Live TV</span>
              </button>
      ...
      <div className="max-w-7xl mx-auto">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-white">
                    Today's {(() => {
                      ...
                      const hours = gmtDate.getUTCHours().toString().padStart(2, '0');
                      const minutes = gmtDate.getUTCMinutes().toString().padStart(2, '0');
                      return `Live Sports Games - ${dayName} ${date}${getOrdinal(date)} ${month} ${year}`;
                    })()}
                  </h2>
                  <p className="text-gray-400 text-sm mt-1">Schedule Time UK GMT</p>
                </div>
              </div>
              <button 
                onClick={() => window.location.hash = '#/sports'}
                className="text-brand-400 hover:text-brand-300 font-medium flex items-center gap-1 text-sm sm:text-base"
              >
                View All <ChevronRight size={16} />
              </button>
            </div>
        ...
        {trending.length > 0 && (
          <div className="bg-dark-surface rounded-xl sm:rounded-2xl p-6 sm:p-8 border border-dark-border mx-4 sm:mx-6 lg:mx-8 mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-white text-center mb-6 sm:mb-8">Why Choose Uwatchfree?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              <div className="text-center group">
                <div className="bg-brand-900/30 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-brand-900/50 transition-colors">
                  <Film className="text-brand-400" size={28} />
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-3">Free Streaming</h3>
                <p className="text-gray-400 text-sm sm:text-base">Watch thousands of movies and TV shows completely free, no subscriptions required.</p>
              </div>
              <div className="text-center group">
                <div className="bg-green-900/30 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-green-900/50 transition-colors">
                  <Zap className="text-green-400" size={28} />
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-3">Live Sports</h3>
                <p className="text-gray-400 text-sm sm:text-base">Never miss a match with live streaming of major sports events from around the world.</p>
              </div>
              <div className="text-center group col-span-1 sm:col-span-2 lg:col-span-1">
                <div className="bg-red-900/30 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-red-900/50 transition-colors">
                  <Wifi className="text-red-400" size={28} />
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-3">Live TV Channels</h3>
                <p className="text-gray-400 text-sm sm:text-base">Access hundreds of live TV channels from various categories including news, sports, and entertainment.</p>
              </div>
            </div>
          </div>
        )}
