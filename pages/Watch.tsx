...
// import { Calendar, Clock, Star, Layers, User, ArrowLeft, Heart, Share2, X, Copy, Check, ExternalLink, Facebook, Twitter, Linkedin, Mail, FileVolume } from 'lucide-react';
...
const Watch: React.FC = () => {
  ...
  // Generate Dynamic Schema for Movie or TV Series
  // STRICT COMPLIANCE: Use integers for ratings, full image array, and include URL.
  const schema = {
    "@context": "https://schema.org",
    "@type": type === 'tv' ? 'TVSeries' : 'Movie',
    "name": title,
    "description": description,
    "image": [getImageUrl(details.poster_path, 'original')], // Array format is preferred by Google
    ...
    "aggregateRating": details.vote_average ? {
      ...
      "ratingValue": details.vote_average.toFixed(1),
      "bestRating": "10",
      "ratingCount": 100 // Must be a Number, not a string
    } : undefined,
    "genre": details.genres?.map(g => g.name),
    "actor": cast.slice(0, 5).map(person => ({
        "@type": "Person",
